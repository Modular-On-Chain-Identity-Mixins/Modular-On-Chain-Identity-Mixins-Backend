...
use soroban_compliance_kit::types::{
    ComplianceAction, ComplianceRule, Jurisdiction, KycStatus, RuleField, RuleOperator,
};
...
fn test_deposit_and_withdraw() {
    ...
    registry_client.add_verifier(&admin, &admin);
    registry_client.update_kyc(&admin, &user1, &KycStatus::Verified);

    let pool_id = env.register(
        DefiPoolContract,
        (
            &token_id, &admin, &registry_id, &1u32,
            &1_000_000_000_000i128, &10_000_000_000_000i128,
            &Vec::<Bytes>::new(&env),
        ),
    );
    ...
    assert_eq!(pool_client.get_pool_config().total_liquidity, 100_000_000_000);

    pool_client.withdraw(&user1, &50_000_000_000i128);
    assert_eq!(pool_client.get_pool_config().total_liquidity, 50_000_000_000);
}
...
fn test_compliance_rule_enforcement() {
    ...
    let rule = ComplianceRule {
        ...
        value: Bytes::from_slice(&env, &[0u8; 15]),
...
fn test_withdraw_insufficient_liquidity_fails() {
    ...
    assert!(pool_client.try_withdraw(&user1, &999_999_999_999_999i128).is_err());
}
...
fn test_deposit_without_kyc_fails() {
    ...
    assert!(pool_client.try_deposit(&user_no_kyc, &100_000_000_000i128).is_err());
}
